...
class Solution {
    ...
    private int solve(String expr, Map<String, Integer> values) {
        List<String> exprList = getExprList(expr);
        if(exprList.get(0).equals("let")) {
            Map<String, Integer> innerValues = new HashMap<>(values);
            for(int i = 1;i < exprList.size() - 2; i += 2) {
                int value = getValueOrSolve(exprList.get(i + 1), innerValues);
                innerValues.put(exprList.get(i), value);
            }
            return getValueOrSolve(exprList.get(exprList.size() - 1), innerValues);
        } else {
            int[] res = new int[2];
            for(int i = 1;i <= 2;i++) {
                res[i - 1] = getValueOrSolve(exprList.get(i), values);
            }

            if(exprList.get(0).equals("add")) {
                return res[0] + res[1];
            } else {
                return res[0] * res[1];
            }
    ...
    private int getValueOrSolve(String expr, Map<String, Integer> values) {
        if(expr.charAt(0) == '(') {
            return solve(expr, values);
        } else if (values.containsKey(expr)){
    ...
    private List<String> getExprList(String expr) {
        List<String> res = new ArrayList<>();
        char[] chars = expr.toCharArray();
        int pre = 1;
        for(int i = 1;i < chars.length - 1;i++) {
            if(chars[i] == ' ') {
                res.add(new String(chars, pre, i - pre));
                pre = i + 1;
            } else if (chars[i] == '(') {
                int k = 0;
        ...
        if(pre < chars.length - 1) {
            res.add(new String(chars, pre, chars.length - 1 - pre));
        }
